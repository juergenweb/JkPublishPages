<?php
    ...
    abstract class IntegrationTestCase extends TestCase
    {
        ...
        public static function setUpBeforeClass(): void
        {
            ...
            if (self::$skipReason !== null) return;
        ...
        {
            if (self::$wire === null || self::$skipReason !== null) return;
            self::loginSuperuser();
            self::removeFixtures();
        }
        ...
        private static function removeFixtures(): void
        {
            ...
            if ($user->id) $wire->users->delete($user);

            $role = $wire->roles->get('name=' . self::ROLE);
            if ($role->id) $wire->roles->delete($role);

            if (self::$createdPublishPermission) {
                $permission = $wire->permissions->get('page-publish');
                if ($permission->id) $wire->permissions->delete($permission);
        ...
        protected static function createPage(array $options = []): Page
        {
            ...
            if (!empty($options['unpublished'])) $page->addStatus(Page::statusUnpublished);
            if (!empty($options['hidden'])) $page->addStatus(Page::statusHidden);
            $page->save();

            $page->of(false);
            if (isset($options['from'])) $page->jk_publish_from = $options['from'];
            if (isset($options['until'])) $page->jk_publish_until = $options['until'];
            $page->jk_action_after = $options['action'] ?? 1;
            if (isset($options['move_to'])) $page->jk_move_child = $options['move_to'];
            $page->save();
