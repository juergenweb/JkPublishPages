<?php
    ...
    final class ValidationTest extends IntegrationTestCase
    {
        ...
        public function testValidation(string $wanted, ?int $startOffset, ?int $endOffset, bool $startError, bool $endError, string $savedStatus, bool $warning): void
        {
            $now = time();
            ...
            $this->assertSame($startError, $result['startErrors'] !== [], 'error on start date field: ' . implode(' ', $result['startErrors']));
            $this->assertSame($endError, $result['endErrors'] !== [], 'error on end date field: ' . implode(' ', $result['endErrors']));
            $this->assertSame($savedStatus, $result['status'], 'status the page will be saved with');
            $this->assertSame($warning, $result['warnings'] !== [], 'warning about the status change: ' . implode(' ', $result['warnings']));
        }
        ...
        public function testErrorForStartInFutureNamesThePast(): void
        {
            ...
            $error = html_entity_decode(implode(' ', $result['startErrors']), ENT_QUOTES);
            $this->assertStringContainsString(mb_strtoupper(html_entity_decode(self::module()->_('past'), ENT_QUOTES)), $error);
        }
        ...
        private function warnings(): array
        {
            ...
            foreach (self::$wire->notices as $notice) {
                if ($notice instanceof NoticeWarning) $texts[] = $notice->text;
            }
            return $texts;
        }
    }
