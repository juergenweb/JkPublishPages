...
final class DecideTest extends TestCase
{
    ...
    public static function provideUnpublishedPages(): array
    {
        ...
        return [
            ...
            'only end in future → nothing (no start = no automatic publishing)' => [
                null, $now + $h, Rules::ACTION_TRASH, Rules::RESULT_NONE,
            ],

            // publication ended, page was unpublished manually before
            'ended, action unpublish → nothing' => [
                $now - 2 * $h, $now - $h, Rules::ACTION_UNPUBLISH, Rules::RESULT_NONE,
            ],
            ...
            'start in future, end in past → nothing' => [
                $now + $h, $now - $h, Rules::ACTION_DELETE, Rules::RESULT_NONE,
            ],
            'start in future, end exactly now → nothing' => [$now + $h, $now, Rules::ACTION_DELETE, Rules::RESULT_NONE],

            // boundaries: exactly now
            'only end exactly now, action unpublish → nothing' => [
                null, $now, Rules::ACTION_UNPUBLISH, Rules::RESULT_NONE,
            ],
            'only end exactly now, action delete → nothing (not ended yet)' => [
                null, $now, Rules::ACTION_DELETE, Rules::RESULT_NONE,
            ],
            'start and end exactly now → publish' => [$now, $now, Rules::ACTION_DELETE, Rules::RESULT_PUBLISH],
            'start exactly now, end in future → publish' => [
                $now, $now + $h, Rules::ACTION_DELETE, Rules::RESULT_PUBLISH,
            ],
            'start exactly now, end in past → nothing (start after end)' => [
                $now, $now - $h, Rules::ACTION_DELETE, Rules::RESULT_NONE,
            ],
            'only end in past, action unpublish → nothing' => [
                null, $now - $h, Rules::ACTION_UNPUBLISH, Rules::RESULT_NONE,
            ],
            'end one second in the past, action trash → trash' => [
                $now - $h, $now - 1, Rules::ACTION_TRASH, Rules::RESULT_TRASH,
            ],
        ];
    }
    ...
    public static function providePublishedPages(): array
    {
        ...
        return [
            ...
            'ended, action unpublish → unpublish' => [
                $now - 2 * $h, $now - $h, Rules::ACTION_UNPUBLISH, Rules::RESULT_UNPUBLISH,
            ],
            'ended, action trash → trash' => [$now - 2 * $h, $now - $h, Rules::ACTION_TRASH, Rules::RESULT_TRASH],
            'ended, action move → move' => [$now - 2 * $h, $now - $h, Rules::ACTION_MOVE, Rules::RESULT_MOVE],
            'ended, action delete → delete' => [$now - 2 * $h, $now - $h, Rules::ACTION_DELETE, Rules::RESULT_DELETE],
            ...
            'start in future, action delete → only unpublish' => [
                $now + $h, $now + 2 * $h, Rules::ACTION_DELETE, Rules::RESULT_UNPUBLISH,
            ],
            'start in future, action trash → only unpublish' => [
                $now + $h, null, Rules::ACTION_TRASH, Rules::RESULT_UNPUBLISH,
            ],
            'start in future, action move → only unpublish' => [
                $now + $h, $now + 2 * $h, Rules::ACTION_MOVE, Rules::RESULT_UNPUBLISH,
            ],
            ...
            'end one second in the past, action delete → delete' => [
                $now - $h, $now - 1, Rules::ACTION_DELETE, Rules::RESULT_DELETE,
            ],
            'only end in past, action unpublish → unpublish' => [
                null, $now - $h, Rules::ACTION_UNPUBLISH, Rules::RESULT_UNPUBLISH,
            ],
        ];
    }
